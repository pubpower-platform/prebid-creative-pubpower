...
var targetHBAdFormat = null;
var adFormatSpecial = ['interstitial', 'anchor'];
/**
 *
...
function newRenderingManager(win, environment) {
  ...
  var renderAd = function renderAd(doc, dataObject) {
    ...
    targetHBAdFormat = dataObject && dataObject.hbAdFormat ? dataObject.hbAdFormat : null;
  ...
  function renderCrossDomain(adId) {
    ...
    function renderAd(ev) {
      ...
      if (adObject.message && adObject.message === 'Prebid Response' && publisherDomain === origin && adObject.adId === adId) {
        try {
    ...
    function requestAdFromPrebid() {
      ...
      }); // this_check_adFormat_Special

      if (targetHBAdFormat && adFormatSpecial.indexOf(targetHBAdFormat) != -1) {
        win.parent.parent.postMessage(message, publisherDomain);
      } else {
